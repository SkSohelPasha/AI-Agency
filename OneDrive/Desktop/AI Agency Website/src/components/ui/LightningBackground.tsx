...
const Lightning: React.FC<LightningProps> = ({
  ...
  useEffect(() => {
    ...
    // Check if all uniform locations are valid
    if (!iResolutionLocation || !iTimeLocation || !uHueLocation || !uXOffsetLocation || !uSpeedLocation || !uIntensityLocation || !uSizeLocation) {
      console.error("One or more uniform locations not found. Check shader code for mismatches.");
      return;
    }
...
const LightningBackground: React.FC<LightningBackgroundProps> = ({ hue }) => {
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
